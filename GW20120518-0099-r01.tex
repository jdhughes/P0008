...
\hspace{15pt}Each kernel launches a grid of threads composed of one or more blocks of grouped threads. The number of blocks in a grid is determined by the size of the kernel data being processed or the number of SM. The total number of threads in a grid is equal to the number of threads per block times the number of blocks. CCs on each SM can run one or more blocks but blocks are restricted to a single CC. Threads within a block are further grouped into warps, which is the maximum number of threads that are managed and processed simultaneously in each block. Multiple warps execute one after another to complete block processing of the kernel. GPGPU architecture and kernel execution are shown graphically in \textbf{figure~\ref{FigParallel}}.
