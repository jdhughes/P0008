...
\hspace{15pt}The total number of threads required to is a CUDA grid. A CUDA grid is composed of one or more blocks that are executed on one or more SM. All threads in a block run on the same SM with a dimension equal to the total number of times the kernel is applied. Each CUDA grid is . For increased throughput, threads within a block are further split into warps, which is the maximum number of threads that are managed and processed simultaneously on each CC. Multiple warps execute one after another to complete block processing of the kernel. All CCs within a SM execute the kernel concurrently, which results in massively parallel processing potential. GPGPU architecture and kernel execution are shown graphically in \textbf{figure~\ref{FigParallel}}.
