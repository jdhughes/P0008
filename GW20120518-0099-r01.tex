...
\label{GPGPUbox2}
\parbox{5.75in}{
\color{blue}\textbf{\\Box 2\\NVIDIA GPGPU Architecture and Kernel Execution}
...
\hspace{15pt}The operations in a CUDA kernel function are distributed as evenly as possible across a CUDA grid, which is a grouping of threads. Each CUDA grid is composed of one or more blocks that execute the kernel function on each CC. For increased throughput, threads within a block are further split into warps, which is the maximum number of threads that are managed and processed simultaneously on each CC. Multiple warps execute one after another to complete block processing. All CCs within a SM execute a single kernel at at time, which results in massively parallel processing potential. GPGPU architecture is shown graphically in  \textbf{figure~\ref{FigParallel}}.
